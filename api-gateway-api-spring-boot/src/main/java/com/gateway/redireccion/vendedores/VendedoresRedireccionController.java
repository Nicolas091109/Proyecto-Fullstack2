...
public class VendedoresRedireccionController {
    ...
    public ResponseEntity<?> proxyProductos(HttpServletRequest request,
        ...
        String targetUrl = "http://localhost:8083/api/vendedores" + originalPath;
        ...
        if (method == HttpMethod.PUT) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede actualizar vendedores\"}");
            }
        }
